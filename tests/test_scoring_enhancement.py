...
class TestScoringEnhancement:
    ...
    def test_scoring_form_generation(self):
        ...
        assert len(scores) == 2
        assert scores["test-problem-1"]["is_correct"]
        assert not scores["test-problem-2"]["is_correct"]
        assert scores["test-problem-2"]["mistake_type"] == "読み間違い"

    def test_attempt_creation_from_scores(self):
        """スコアデータから試行データ作成テスト"""
        scores = {
            "test-problem-1": {"is_correct": True, "mistake_type": None, "notes": "テストメモ1"},
            "test-problem-2": {
                "is_correct": False,
                "mistake_type": "読み間違い",
                "notes": "テストメモ2",
        ...
        for problem_id, score_data in scores.items():
            attempt = Attempt(problem_id=problem_id, is_correct=score_data["is_correct"])
        ...
        assert not attempts[1].is_correct
    ...
    def test_empty_printed_problems_handling(self):
        ...
        if not printed_problems:
            ...
            assert show_manual_selection

    def test_scoring_form_validation(self):
        """採点フォームバリデーションテスト"""
        # 有効なスコアデータ
        valid_scores = {"problem-1": {"is_correct": True, "mistake_type": None, "notes": "正解"}}

        # 無効なスコアデータ（is_correctが欠如）
        invalid_scores = {"problem-1": {"mistake_type": "読み間違い", "notes": "間違い"}}

        # 有効なデータの検証
        for _problem_id, score_data in valid_scores.items():
            assert "is_correct" in score_data
            assert isinstance(score_data["is_correct"], bool)

        # 無効なデータの検証
        for _problem_id, score_data in invalid_scores.items():
