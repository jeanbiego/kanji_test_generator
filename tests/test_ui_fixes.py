...
from src.app import show_problem_creation_page
...
class TestUIFixes:
    ...
    def test_reset_button_functionality(self):
        ...
        with patch.object(st, "session_state") as mock_session:
            mock_session.__contains__ = lambda key: key in [
                "duplicate_detected",
                "duplicate_message",
                "pending_problem",
            ]
    ...
    def test_print_page_css_embedding(self):
        """印刷用ページのCSS埋め込みテスト"""
        # Arrange
        test_problems = [Problem(sentence="テスト問題文", answer_kanji="テスト", reading="テスト")]
