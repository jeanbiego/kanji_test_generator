...
class TestHistoryEnhancement:
    ...
    def setup_method(self):
        ...
        self.test_problems = [
            ...
            Problem(
                ...
                answer_kanji="景色",
                reading="ケシキ",
                created_at=datetime(2025, 1, 27, 11, 0, 0)
            )
        ]
        ...
        self.test_attempts = [
            Attempt(
                ...
                attempted_at=datetime(2025, 1, 27, 12, 0, 0),
                is_correct=True
            ),
            Attempt(
                id="attempt-2",
                problem_id="test-problem-1",
                attempted_at=datetime(2025, 1, 27, 13, 0, 0),
                is_correct=False
            ),
            Attempt(
                id="attempt-3",
                problem_id="test-problem-2",
                attempted_at=datetime(2025, 1, 27, 14, 0, 0),
                is_correct=True
            )
        ]
    ...
    def test_problem_statistics_calculation(self):
        ...
        assert problem_stats['test-problem-1']['last_attempted'] == datetime(2025, 1, 27, 13, 0, 0)
        
        # 問題2の統計
        assert problem_stats['test-problem-2']['correct_count'] == 1
        assert problem_stats['test-problem-2']['total_count'] == 1
        assert problem_stats['test-problem-2']['last_attempted'] == datetime(2025, 1, 27, 14, 0, 0)
    ...
    def test_last_attempted_date_formatting(self):
        """最後の試行日フォーマット機能テスト"""
        last_attempted = datetime(2025, 1, 27, 14, 30, 0)
    ...
    def test_problem_title_with_statistics(self):
        ...
        problem_stat = {
            'correct_count': 1,
            'total_count': 2,
            'last_attempted': datetime(2025, 1, 27, 13, 0, 0)
        }
    ...
    def test_statistics_display_format(self):
        ...
        problem_stat = {
            'correct_count': 3,
            'total_count': 5,
            'last_attempted': datetime(2025, 1, 27, 15, 0, 0)
        }
