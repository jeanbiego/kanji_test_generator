...
## 最新の修正内容
- **縦書き対応**: 印刷用ページを縦書き既定に変更、問題番号を漢数字（問一、問二…）で表示
- **解答欄レイアウト**: 長方形を縦長ボックスに統一（回転を廃止）
- **印刷フッター**: Page 1/1 の表記を非表示
- **余白調整**: 問題間の余白を拡大し視認性向上
- **Chrome印刷最適化**: 固定mm指定を撤廃し、改ページ抑止スタイルを追加して空白2ページ発生を抑制
- **抽出数反映**: 「1ページあたりの問題数」を抽出件数にも反映（5固定を解消）
...
### 追加課題（2025年9月23日 - 第3弾）
- [x] **問題登録ページ**: 重複判定でエラーが出ない場合の「入力内容は正常です」表示を削除（正常状態の明示は不要のため）
- [x] **問題登録ページ（レビュー表示）**: レビュー表示形式を「プレビュー: 〜」に変更（読みをカタカナ表示）
- [x] **問題用紙作成ページ**: 「印刷対象の問題」に「不正解数」を追加
- [x] **履歴管理ページ**: 表示される問題情報に「不正解数」を追加
- [x] **採点ページ**: 「採点」見出しの上に出ている問題一覧を非表示（見出し下のフォームのみ表示）
- [x] **印刷用ページ**: 「答え：」横の長方形（解答欄）の縦サイズを1.5倍に拡大（視認性向上のため）

（完了日: 2025-09-23 / タグ: v1.9.2）

### 追加課題（2025年9月27日 - 第4弾）
- [x] **問題登録ページ**: 新規問題追加時の不正解数カウントが増えない問題（＋１されるのが正しい）
- [x] **問題保存時**: 保存成功などの表示がでない問題（ユーザーフィードバックの改善）

（完了日: 2025-09-27 / 状態: 実装完了、テスト完了）

### 追加修正（2025-09-23 / v1.9.3）
- **データクレンジング**: `problems.csv`のID重複を除去・`incorrect_count`を補完、`attempts.csv`は有効な`problem_id`のみに整合
- **ストレージ**: 既存IDのみ更新保存（未登録IDは追加しない）、部分削除APIで重複IDの1件削除に対応
- **履歴UI**: 重複IDを非表示（最初の1件のみ）、操作ボタンのキー一意化
...
- **印刷用ページ（縦書き対応）**: CSSで縦書き表示にする
  - 仕様: `.print-content` または問題テキスト領域に `writing-mode: vertical-rl; text-orientation: mixed;` を適用
  - 影響: 問題テキスト・番号・解答欄のレイアウト再調整が必要
- **印刷レイアウト（横向きデフォルト）**: A4横向きをデフォルト設定
  - 仕様: `@page { size: A4 landscape; }` を印刷CSSに設定
  - 備考: ブラウザの印刷ダイアログの設定が優先される場合があるため、ガイド文も追記予定

※ 本項目は仕様追記のみ。実装は別コミットで対応する。

### 追加課題（2025年1月27日 - 第5弾）
- [ ] **問題抽出数の表示不整合**: 抽出数を7に設定したとき、漢字は正しく7問抽出されているのに「印刷対象の問題 (6問)」など表記の数字が間違っているときがある
  - **詳細**: 抽出機能は正常に動作しているが、表示される問題数のカウントが不正確
  - **影響範囲**: 問題用紙作成ページの抽出結果表示
  - **優先度**: 中
  - **影響度**: 中 - ユーザビリティに影響

- [x] **印刷時の空白ページ問題**: 7問以上抽出すると、問題自体は1ページに収まっているのに、空白だけの2ページ目が印刷される
  - **詳細**: 印刷レイアウトの改ページ制御に問題がある可能性
  - **影響範囲**: 印刷用ページのレイアウト
  - **優先度**: 中
  - **影響度**: 中 - 印刷品質に影響
  - **技術的詳細**: CSSの改ページ制御（`page-break-inside: avoid`等）の見直しが必要
  - **修正内容**: 
    - 印刷用CSSの空白ページ対策を根本的に見直し
    - HTMLテンプレートの構造を最適化
    - ページ分割ロジックを完全に再設計
    - 7問以下の場合は1ページにまとめる仕組みを追加
    - @page設定でorphans/widowsを0に設定
    - 高さ制限を撤廃してoverflow: visibleに変更
  - **判明したこと**:
    - 縦書きレイアウト（writing-mode: vertical-rl）が高さを大幅に増加させる
    - 問題アイテムの余白（margin-bottom: 18mm）が累積して高さを増加
    - フォントサイズや余白の大幅削減は可読性を損なう
    - 根本的な解決には問題数の制限が最も安全
  - **代替解決策**:
    - 7問以上の場合、自動的に6問に制限
    - 印刷前に「1ページに収まりますか？」の確認表示
    - 段階的な問題表示（最初の6問のみ）

- [ ] **重複判定の不具合**: 回答漢字と読みが同じ問題なのに、作成時に重複判定をパスしてしまうことがある
  - **詳細**: 重複チェック機能が正常に動作していない場合がある
  - **影響範囲**: 問題登録ページの重複チェック機能
  - **優先度**: 高
  - **影響度**: 高 - データ整合性に直接影響
  - **技術的詳細**: 重複判定ロジックの見直しが必要（完全一致チェック、漢字・読み組み合わせチェック、問題文一致チェックの再検証）
