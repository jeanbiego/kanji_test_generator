        ...
        with st.spinner("アプリケーションを初期化しています..."):
            try:
                ...
                health_result = run_health_check(
                    st.session_state.problem_storage, st.session_state.attempt_storage
                )
                ...
                if health_result.has_issues:
                    ...
                    st.info(
                        "💡 データクリーニングスクリプトの実行を推奨します: `python scripts/clean_data.py`"
                    )
...
def show_problem_creation_page():
    ...
    if st.session_state.problem_review_mode:
        ...
        validation_result = validator.validate_problem(
            problem_data["sentence"], problem_data["answer_kanji"], problem_data["reading"]
        )
...
def check_duplicate_problem(sentence: str, answer_kanji: str, reading: str) -> tuple[bool, str]:
    ...
    try:
        ...
        for problem in saved_problems:
            ...
            if problem.answer_kanji == answer_kanji and problem.reading == normalized_reading:
                return (
                    True,
                    f"同じ漢字・読みの組み合わせが既に存在します（問題文: {problem.sentence[:30]}...）が、保存することもできます",
                )
...
def show_print_page():
    ...
    with col_set1:
        total_questions = st.number_input(
            "総問題数", min_value=1, max_value=100, value=10, help="印刷する問題の総数を設定します"
        )
    with col_set2:
        title = st.text_input(
            "テストタイトル", value="漢字テスト", help="印刷用ページのタイトルを設定します"
        )
        ...
        if st.button("🎯 苦手上位", type="primary", use_container_width=True):
            try:
                ...
                if not saved_problems:
                    st.warning(
                        "保存された問題がありません。問題登録ページで問題を作成してください。"
                    )
                    return
                ...
                sorted_problems = sorted(
                    problems_with_incorrect_count, key=lambda x: x[1], reverse=True
                )
    ...
    for i, problem in enumerate(problems_to_print):
        with st.expander(
            f"問題 {i + 1}: {problem.answer_kanji} ({problem.reading}) / 不正解数: {problem.incorrect_count}"
        ):
    ...
    if st.button("🖨️ 印刷用ページを表示", type="primary"):
        try:
            ...
            if total_pages > 1:
                st.info(
                    f"📄 {len(problems_to_print)}問を{total_pages}ページに分割して表示します（1ページあたり10問）"
                )
            else:
            ...
            for i, problem in enumerate(st.session_state.extracted_problems):
                ...
                with col1:
                    correct = st.radio(
                        "正誤",
                        ["正解", "不正解"],
                        key=f"printed_score_{problem.id}",
                        horizontal=True,
                    )
                with col2:
                    if correct == "不正解":
                ...
                with col3:
                    notes = st.text_input(
                        "メモ", key=f"printed_notes_{problem.id}", placeholder="学習メモ（任意）"
                    )
            ...
            if submitted:
                try:
                    ...
                    for problem_id, score_data in scores.items():
                        attempt = Attempt(
                            problem_id=problem_id, is_correct=score_data["is_correct"]
                        )
                            ...
                            for score in scores.values():
                                if not score["is_correct"] and score["mistake_type"]:
                                    ...
                                    mistake_analysis[mistake_type] = (
                                        mistake_analysis.get(mistake_type, 0) + 1
                                    )
    ...
    try:
        ...
        with col2:
            sort_by = st.selectbox(
                "📊 並び順", ["作成日時（新しい順）", "作成日時（古い順）", "苦手（不正解数順）"]
            )
        ...
        if search_term:
            ...
            filtered_problems = [
                ...
                if (
                    search_term in p.sentence.lower()
                    or search_term in p.answer_kanji.lower()
                    or search_term in p.reading.lower()
                )
            ]
        ...
        # 基本情報の表示
        st.write(
            f"**総問題数**: {len(saved_problems)} | **表示中**: {len(display_problems)} | **検索結果（重複含む）**: {len(filtered_problems)} | **一意ID数**: {len(unique_problems)}"
        )
            ...
            for i, problem in enumerate(display_problems):
                ...
                with st.expander(title):
                    ...
                    with col1:
                        ...
                        st.write(f"**読み**: {problem.reading}")
                        st.write(
                            f"**作成日時**: {problem.created_at.strftime('%Y年%m月%d日 %H:%M')}"
                        )
                        st.write(f"**不正解数**: {problem.incorrect_count}")
                    ...
                    with col2:
                        ...
                        with col_btn1:
                            if st.button(
                                "📄 印刷", key=f"print_{i}_{problem.id}_{hash(problem.sentence)}"
                            ):
                        ...
                        with col_btn2:
                            if st.button(
                                "✏️ 採点", key=f"score_{i}_{problem.id}_{hash(problem.sentence)}"
                            ):
                        ...
                        # 削除ボタン
                        if st.button(
                            "🗑️ 削除", key=f"delete_{i}_{problem.id}_{hash(problem.sentence)}"
                        ):
                            # 重複が存在する場合でも1件のみ削除
                            if hasattr(
                                st.session_state.problem_storage, "delete_problem_once"
                            ) and st.session_state.problem_storage.delete_problem_once(problem.id):
